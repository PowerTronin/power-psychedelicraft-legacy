...
public interface PSBlocks {
    ...
    NightshadeBlock JIMSONWEED = register("jimsonweed", new NightshadeBlock(
    ...
    Block GLASS_VALVE = register("glass_valve", new ValveBlock(Settings.create().mapColor(MapColor.OFF_WHITE).strength(0.3F).sounds(BlockSoundGroup.GLASS).nonOpaque()
            ...
            .suffocates(Blocks::never)
            .blockVision(Blocks::never)));
    Block PUMP = register("pump", new PumpBlock(Settings.create().mapColor(MapColor.STONE_GRAY).instrument(Instrument.BASEDRUM).requiresTool().strength(3.5F)));
    ...
    Block CAULDRON = register("cauldron", new FluidCauldronBlock(Settings.copy(Blocks.CAULDRON)));
