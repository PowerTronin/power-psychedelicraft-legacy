...
public class DrugProperties implements NbtSerialisable {
    ...
    public boolean rollStroke() {
        ...
        PSDamageTypes.damage(entity.getWorld(), entity, damageOf(PSDamageTypes.STROKE), Math.min(baseDamage + strokeIntensity, 10));
    ...
    public void onTick() {
        ...
        if (cancerCountdown >= 0 && cancerCountdown <= 10 && --cancerCountdown == 0) {
            prevHadCancer = false;
            cancerCountdown = -1;
            PSDamageTypes.damage(entity.getWorld(), entity, damageOf(PSDamageTypes.CANCER), Integer.MAX_VALUE);
        }
        ...
        if (entity.getWorld().isClient) {
            ...
            if (cardiacArrestTicks > MAX_CARDIAC_ARREST_TIME) {
                ...
                if (random.nextInt(200) == 0 || entity.isSleeping()) {
                    cardiacArrestTicks = 0;
                    PSDamageTypes.damage(entity.getWorld(), entity, damageOf(PSDamageTypes.HEART_ATTACK), Integer.MAX_VALUE);
                }
            }
        }
    ...
    public void sendCapabilities() {
        if (!entity.getWorld().isClient) {
            var message = new MsgDrugProperties(this);
    ...
    public NbtCompound toTrackedNbt(NbtCompound compound) {
        dirty = false;
        ...
        if (initial) {
            compound.put("soundManager", soundManager.toNbt());
            initial = false;
        }
    ...
    public void fromTrackedNbt(NbtCompound compound) {
        dirty = false;
        drugs.clear();
        DRUGS_CODEC.decode(NbtOps.INSTANCE, compound.getCompound("Drugs")).result().map(Pair::getFirst).ifPresent(drugs::putAll);
        influences.clear();
        DrugInfluenceInstance.LIST_CODEC.decode(NbtOps.INSTANCE, compound.getList("drugInfluences", NbtElement.COMPOUND_TYPE)).result().map(Pair::getFirst).ifPresent(influences::addAll);
        stomach.fromNbt(compound.getCompound("stomach"));
        ...
        if (compound.contains("soundManager", NbtElement.COMPOUND_TYPE)) {
            soundManager.fromNbt(compound.getCompound("soundManager"));
        }
    }

    @Override
    public void fromNbt(NbtCompound compound) {
        fromTrackedNbt(compound);
        initial = true;
    }

    @Override
    public void toNbt(NbtCompound compound) {
        toTrackedNbt(compound);
    }
